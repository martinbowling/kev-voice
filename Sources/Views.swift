        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                Picker("Settings section", selection: $model.settingsTab) {
                    ...
                    Text("Kev activity").tag("Kev activity")
                }.pickerStyle(.segmented).labelsHidden()
                if model.settingsTab == "Kev activity" { KevActivityView(model: model) }
                else {
                    setup
                }
            }.padding(24)
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            Card {
                VStack(alignment: .leading, spacing: 12) {
                    Label("Local Kev model", systemImage: "brain").font(.system(size: 15, weight: .medium))
                    Text("Decisions run on this Mac through the local Kev server. Start it with kev/run.sh.").font(.system(size: 12)).foregroundStyle(Palette.muted)
                    Text("Server: \(KevService.displayAddress)").font(.system(size: 11, design: .monospaced)).foregroundStyle(Palette.accent).textSelection(.enabled)
                    Text("Model: \(KevService.model)").font(.system(size: 11, design: .monospaced)).foregroundStyle(Palette.accent)
                    connectionActions
            ...
            Card {
                VStack(alignment: .leading, spacing: 9) {
                    ...
                    Text("Nothing. The local Kev server receives your transcript, app/window title, page URL, visible UI text and field values, and action labels to choose the next action. Audio, screen text and decisions stay on this Mac. Password fields are excluded. Kev activity shows the actual request and response bodies. No API keys or accounts are used.").font(.system(size: 12)).foregroundStyle(Palette.muted).fixedSize(horizontal: false, vertical: true)
...
struct CommandBarView: View {
    ...
    private var status: String {
        ...
        if !model.accessibilityGranted { return "Finish setup in Settings" }
