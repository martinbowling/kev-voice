...
enum SelfTests {
    static func run() {
        precondition(KevClient.model == "kev-latest")
        precondition(KevService.endpoint.absoluteString.hasSuffix("/v1/systemone"))
        precondition(KevService.status.absoluteString.hasSuffix("/v1/models"))
        let serverError = Data(#"{"error":{"message":"question has no criteria"}}"#.utf8)
        precondition(KevClient.serverMessage(from: serverError) == "question has no criteria")
        let validationError = Data(#"{"detail":"criteria must have 1..255 options"}"#.utf8)
        precondition(KevClient.serverMessage(from: validationError) == "criteria must have 1..255 options")
        precondition(KevClient.serverMessage(from: Data("not json".utf8)) == nil)
        print("PASS: local Kev endpoint, model label and server error parsing.")
        ...
        print("PASS: all catalogue options preserved, Unicode literal spans, invalid-span rejection, physical keys/modifiers, no typing without a focused field, local model identity.")
        speech()
        continuous()
    }
    static func speech() {
    ...
    @MainActor static func router() async {
        ...
        do {
            for (request, screen, history, expected, choices) in cases {
                let result = try await client.select(options: choices, request: request, screen: screen, history: history)
            ...
            let result = try await client.select(options: many, request: "Click Station 397", screen: "Dashboard with numbered station buttons.", history: "No previous actions")
            ...
            let text = try await ActionParameters(client: client, controller: MacController()).text(request: command, snapshot: scene,
                history: "Opened Practice Lab; clicked Coral; focused Practice text. Next action: insert the requested words, then later press Tab.")
            results.append(["case": "Model-selected literal span without command parser", "selected_text": text, "passed": text == "moonstone river"])
            let keyboard = try await ActionParameters(client: client, controller: MacController()).keyboard(request: "Press Command+A, type velvet moon, and press Arrow Left.", snapshot: scene, history: "No actions yet. The field is focused and contains old text.")
            ...
            let report: [String: Any] = ["requested_model": KevClient.model, "scope": "Local Kev selection tests with synthetic UI state; no UI execution", "results": results, "exchanges": exchanges]
            ...
            print("PASS: \(results.count) live local-Kev selection cases, including full-request progression, literal-span selection and 420-option coverage.")
