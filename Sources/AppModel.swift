...
@MainActor final class AppModel: ObservableObject {
    ...
    @Published private(set) var kevHistory = KevCallHistory()
    ...
    init() {
        ...
        client.onActivity = { [weak self] record in self?.kevHistory.receive(record) }
        ...
        Task { [weak self] in
            guard let self else { return }
            do {
                let status = try await KevClient().serviceStatus()
                let device = status["device"] as? String ?? "unknown device"
                self.connectionDetail = "Local Kev server connected on \(device)."
            } catch { self.connectionDetail = error.localizedDescription }
        }
    ...
    func checkConnection() {
        ...
        connectionDetail = "Checking the local Kev server…"
        Task {
            var connected = false
            do {
                let probe = KevClient()
                probe.activityStage = "Connection check"
                probe.onActivity = { [weak self] record in self?.kevHistory.receive(record) }
                let status = try await probe.serviceStatus()
                let device = status["device"] as? String ?? "unknown device"
                let backend = status["backend"] as? String
                let run = status["run"] as? String ?? KevService.model
                let result = try await probe.checkConnection()
                guard result.actionID == "ready" else { throw VoiceError.message("The local Kev model did not confirm the connection.") }
                cost += result.cost
                resolvedModel = result.model
                connectionDetail = "Connected to \(run) on \(device)" + (backend.map { " via \($0)" } ?? "") + ". Repeat your request when ready."
    ...
    private func continueSession() {
        DispatchQueue.main.async { [weak self] in
            ...
            else if self.micEnabled { self.phase = "Listening" }
        }
    }
    ...
    func run(_ command: String, fromSpeech: Bool) {
        guard !busy, !checkingConnection else { return }
        guard AXIsProcessTrusted() else { detail = "Enable Kev Voice in macOS Accessibility to control apps."; showSetup = true; showMain?(); return }
        ...
        showOverlay?()
        beforeRequest?()
        operation = Task { await performRequest(command: command, token: token, start: start) }
    }
    private func performRequest(command: String, token: Int, start: Date) async {
            ...
            do {
                ...
                for iteration in 0..<80 {
                    ...
                    let decision = try await client.select(options: actions.map { ChoiceOption(id: $0.id, description: $0.detail, summary: $0.title, direct: $0.isDirectChoice, category: $0.category) },
                        request: command, screen: snapshot.summary, history: workflow)
                    try Task.checkCancellation()
                    guard token == generation else { return }
                    ...
                    if case .none = action.kind {
                        throw VoiceError.message("The local model could not find a supported next action for the complete request. Nothing further was done.")
                    }
                    if case .complete = action.kind {
                        ...
                        if verified.actionID != "verified" || audit["literal_values"]?.actionID != "correct" {
                            kevHistory.outcome(for: verified.requestID, "Completion rejected by the independent check. The task continues.")
                        ...
                        kevHistory.outcome(for: verified.requestID, "Completion check accepted the newly observed screen after \(completedActionCount) actions.\n\(finalScreen.summary)")
                        detail = "Finished · \(completedActionCount) actions · \(apiCallCount) model calls."
                        totalMS = Date().timeIntervalSince(start) * 1000
                    ...
                    do {
                        ...
                        case .typeText, .replaceText:
                            ...
                            let payload = try await parameters.text(request: command, snapshot: snapshot, history: workflow)
                        ...
                        case .keyboard:
                            ...
                            let (name, code, flags) = try await parameters.keyboard(request: command, snapshot: snapshot, history: workflow)
                        ...
                        case .drag:
                            ...
                            let (source, destination) = try await parameters.drag(request: command, snapshot: snapshot, history: workflow)
                        ...
                        try Task.checkCancellation()
                        kevHistory.outcome(for: decision.requestID, "Execution failed: \(executedTitle). \(error.localizedDescription)")
                    ...
                    kevHistory.outcome(for: decision.requestID, "Executed: \(executedTitle)\n\(outcome)\n\(effect)\n\nObserved afterward:\n\(after.summary)")
                    ...
                    // Every successful action returns to fresh observation and a model decision.
                    try await Task.sleep(nanoseconds: 180_000_000)
                }
                throw VoiceError.message("The request reached its action limit before the local model could verify completion.")
            } catch {
    ...
    private func savePickerReport(success: Bool, error: String?) {
        ...
        var report: [String: Any] = ["architecture": "Live accessibility action picker; no task recipes", "original_request": activeCommand,
            "success": success, "requested_model": KevClient.model, "api_calls": apiCallCount,
            "rounds": pickerSteps, "api_exchanges": pickerExchanges, "milliseconds": totalMS]
        if let error { report["error"] = error }
        let destination = Bundle.main.bundleURL.deletingLastPathComponent().appendingPathComponent("kev-voice-picker-last-run.json")
    ...
    private func saveReplayReport(execute: Bool) {
        let report: [String: Any] = ["source": replaySource + " at real-time pace", "browser_substitution": "none; requests are preserved verbatim", "executed": execute, "on_device": true, "utterances": replayCommands, "results": replayResults, "events": replayEvents, "action_evidence": replayEvidence, "elapsed_seconds": Date().timeIntervalSince(replayStart), "requested_model": KevClient.model]
        let destination = Bundle.main.bundleURL.deletingLastPathComponent().appendingPathComponent(execute ? "kev-voice-demo-execution.json" : "kev-voice-demo-recognition.json")
