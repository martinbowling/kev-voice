...
/// Allows editing without activating Kev Voice over the app being controlled.
...
@MainActor final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        window.title = "Kev Voice Settings"
        ...
        overlay.title = "Kev Voice command bar"
        ...
        statusItem.button?.image = NSImage(systemSymbolName: "waveform", accessibilityDescription: "Kev Voice")
        let menu = NSMenu()
        for (title, selector, key) in [("Show command bar", #selector(showCommandCenter), ""), ("Settings & diagnostics…", #selector(showSettings), ""), ("Toggle microphone  ⌥ Space", #selector(listen), ""), ("Hide command bar", #selector(hideCommandBar), ""), ("Quit Kev Voice", #selector(quit), "q")] {
        ...
        appMenu.addItem(withTitle: "Quit Kev Voice", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
        ...
        if model.microphoneGranted && model.speechGranted && model.accessibilityGranted {
        ...
        Timer.scheduledTimer(withTimeInterval: 0.3, repeats: true) { [weak self] _ in
            MainActor.assumeIsolated {
                guard let self else { return }
                self.statusItem.button?.image = NSImage(systemSymbolName: self.model.busy ? "waveform.circle.fill" : self.model.micEnabled ? "mic.fill" : "mic.slash", accessibilityDescription: "Kev Voice: " + self.model.phase)
                self.statusItem.button?.toolTip = "Kev Voice · " + self.model.phase + " · " + self.model.detail
            }
        }
    }
    ...
    @objc func showPractice() {
        if practice == nil {
            ...
            practice?.title = "Kev Voice · Practice"
