...
// Local HTTP fixtures exercise the real KevClient lifecycle without computer actions.
final class ActivityFixtureProtocol: URLProtocol {
    ...
    override func startLoading() {
        ...
        let status = Self.scenario == "invalid_request" ? 400 : Self.scenario == "recover" && Self.attempts == 1 ? 503 : 200
        let body: String
        switch Self.scenario {
        case "routing":
            let probabilities = Self.routingCriteria.mapValues { _ in 0.0 }.merging([Self.routingChoice: 1.0]) { _, new in new }
            let object: [String: Any] = ["model": "kev-latest", "answers": ["next_action": ["type": "choice", "choice": Self.routingChoice, "confidence": 1.0, "probabilities": probabilities]]]
            body = String(data: try! JSONSerialization.data(withJSONObject: object), encoding: .utf8)!
        case "invalid_request": body = #"{"detail":"criteria must have 1..255 options"}"#
        case "malformed": body = "not a JSON response"
        case "invalid_choice": body = #"{"model":"kev-latest","answers":{"next_action":{"type":"choice","choice":"absent","confidence":1,"probabilities":{"absent":1}}}}"#
        default: body = #"{"model":"kev-latest","answers":{"next_action":{"type":"choice","choice":"yes","confidence":0.9,"probabilities":{"yes":0.9,"no":0.1}}}}"#
        }
...
enum ActivityTests {
    @MainActor static func run() async {
        ...
        let displayed = KevCallRecord.formatted(Data(raw.utf8))
        ...
        for scenario in ["success", "invalid_request", "malformed", "invalid_choice", "timeout", "cancelled"] {
            ActivityFixtureProtocol.scenario = scenario
            let config = URLSessionConfiguration.ephemeral
            config.protocolClasses = [ActivityFixtureProtocol.self]
            let client = KevClient(configuration: config, maxRetries: 0)
            var events: [KevCallRecord] = []
            ...
            precondition(events[1].input.contains("café 👋"))
            precondition(exchanges.count == 1 && exchanges[0]["input"] != nil)
            precondition((events[1].error == nil) == (scenario == "success"))
            if scenario == "invalid_request" { precondition(events[1].httpStatus == 400 && events[1].error?.contains("criteria") == true) }
        ...
        let recovery = KevClient(configuration: recoveryConfig)
        var retries: [KevCallRecord] = []
        recovery.onActivity = { if !$0.pending { retries.append($0) } }
        do {
            let answer = try await recovery.choose(transcript: "recover", context: "fixture", criteria: ["yes": "Yes", "no": "No"])
        ...
        ActivityFixtureProtocol.scenario = "invalid_request"; ActivityFixtureProtocol.attempts = 0
        do { _ = try await recovery.choose(transcript: "invalid_request", context: "fixture", criteria: ["yes": "Yes", "no": "No"]); preconditionFailure() } catch {}
        precondition(ActivityFixtureProtocol.attempts == 1)
        print("PASS: transient failures recover automatically; rejected requests never retry; each attempt is traced.")
        ActivityFixtureProtocol.scenario = "routing"; ActivityFixtureProtocol.attempts = 0
        let routing = KevClient(configuration: recoveryConfig)
        ...
        do {
            let answer = try await routing.select(options: leaves, request: "Open application 9", screen: "Desktop", history: "No actions")
        ...
        var history = KevCallHistory(capacity: 2)
        var first = KevCallRecord(stage: "First", command: "one", input: "{}", optionCount: 2)
        var second = KevCallRecord(stage: "Second", command: "two", input: "{}", optionCount: 2)
        ...
        let third = KevCallRecord(stage: "Third", command: "three", input: "{}", optionCount: 2)
