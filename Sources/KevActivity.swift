...
struct KevCallRecord: Identifiable {
    let id = UUID()
    var number = 0
    let startedAt = Date()
    let stage: String
    ...
    var status: String {
        if pending { return "Waiting for Kev" }
...
struct KevCallHistory {
    private(set) var calls: [KevCallRecord] = []
    ...
    mutating func receive(_ record: KevCallRecord) {
...
struct KevActivityView: View {
    @ObservedObject var model: AppModel
    @State private var selectedID: UUID?
    @State private var followLatest = true
    @State private var bodyTab = "Input"
    private var selected: KevCallRecord? {
        if followLatest { return model.kevHistory.calls.last }
        return model.kevHistory.calls.first { $0.id == selectedID } ?? model.kevHistory.calls.last
    }
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                Label("Kev input & output", systemImage: "arrow.left.arrow.right").font(.system(size: 17, weight: .semibold))
                Spacer()
                Toggle("Follow latest", isOn: Binding(get: { followLatest }, set: { enabled in
                    if !enabled { selectedID = model.kevHistory.calls.last?.id }
                    followLatest = enabled
                })).toggleStyle(.checkbox).font(.system(size: 11))
            }
            Text("Every local Kev call, including option batches, typing choices and completion checks. Last 300 calls from this session; nothing is sent off this Mac.")
                .font(.system(size: 11)).foregroundStyle(Palette.muted).fixedSize(horizontal: false, vertical: true)
            if model.kevHistory.calls.isEmpty {
                Card {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        Button("Check connection") { model.checkConnection() }
                            .disabled(model.busy || model.checkingConnection)
                    }
                }
            } else {
                ScrollView {
                    LazyVStack(spacing: 4) {
                        ForEach(model.kevHistory.calls.reversed()) { call in
                            Button {
                            ...
                            }.buttonStyle(.plain).accessibilityLabel("Kev call \(call.number): \(call.stage), \(call.status)")
                        }
                    }
                }.frame(height: CGFloat(min(3, model.kevHistory.calls.count) * 54))
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        HStack {
                            Picker("Request or response", selection: $bodyTab) {
                                Text("Input to Kev").tag("Input")
                                Text("Output from Kev").tag("Output")
                        ...
                        JSONBodyView(text: bodyText(call), label: bodyTab == "Input" ? "Full request JSON" : bodyTab == "Output" ? "Full response JSON" : "Observed action result")
                            .frame(height: 300).clipShape(RoundedRectangle(cornerRadius: 9))
                        Text("POST " + KevClient.endpoint.absoluteString).font(.system(size: 9, design: .monospaced)).foregroundStyle(Palette.muted)
                    }
                }
            }
        }
    }
    private func bodyText(_ call: KevCallRecord) -> String {
        ...
        return call.output ?? (call.pending ? "Waiting for Kev…" : "No response body was received.\n" + (call.error ?? ""))
    }
}
